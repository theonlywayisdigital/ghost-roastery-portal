...
export function GreenBeanForm({
  ...
  const hasDetailData = !!(bean?.origin_region || bean?.variety || bean?.process || bean?.lot_number ||
    bean?.supplier_id || bean?.arrival_date || bean?.cupping_score || bean?.tasting_notes ||
    bean?.altitude_masl || bean?.harvest_year || bean?.notes);
  const [showDetails, setShowDetails] = useState(hasDetailData);
  ...
  async function handleDelete() {
    ...
    if (res.ok) { router.push("/inventory/green"); router.refresh(); }
  }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200">
          <button type="button" onClick={() => setShowDetails(!showDetails)} className="flex items-center justify-between w-full p-6">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-semibold text-slate-900">More Details</h2>
              {!showDetails && (form.variety || form.process || form.origin_region) && (
                <span className="text-xs text-slate-500">
                  {[form.origin_region, form.variety, form.process].filter(Boolean).join(", ")}
                </span>
              )}
            </div>
