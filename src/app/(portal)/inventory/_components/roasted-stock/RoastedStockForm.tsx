...
export function RoastedStockForm({
  ...
  async function handleDelete() {
    ...
    if (res.ok) { router.push("/inventory/roasted"); router.refresh(); }
  }
    ...
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-slate-900">{isEdit ? "Edit Roast Profile" : "New Roast Profile"}</h1>
        </div>
        {isEdit && (
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href="/inventory/roasted" className="px-4 py-2.5 text-sm text-slate-600 hover:text-slate-800 transition-colors">Cancel</Link>
        </div>
      </form>
    </div>
  );
