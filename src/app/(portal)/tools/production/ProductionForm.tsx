...
export function ProductionForm({
  ...
  async function handleDelete() {
    ...
    if (res.ok) { router.push("/tools/production"); router.refresh(); }
  }

  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/tools/production" className="p-2 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100 transition-colors">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href="/tools/production" className="px-4 py-2.5 text-sm text-slate-600 hover:text-slate-800 transition-colors">Cancel</Link>
        </div>
      </form>
    </div>
  );
