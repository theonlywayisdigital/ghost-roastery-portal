...
export async function POST(req: NextRequest) {
  ...
  // Credit check
  const creditCheck = await checkAiCredits(roaster.id as string, "generate_automation");
...
Return a JSON object with this EXACT structure:
{
  ...
  "steps": [
    ...
    {
      "step_type": "condition",
      "config": { "field": "opened_previous", "yes_action": "continue", "no_action": "end_automation" }
    }
  ],
...
Each condition has a "field" and two branch actions: "yes_action" (when condition is true) and "no_action" (when false).
Branch action options: "continue" (advance to next step), "end_automation", "change_contact_type", "change_pipeline_stage"
If using change_contact_type or change_pipeline_stage, also add "yes_action_value" or "no_action_value" with the target value.

### Condition fields:
- has_placed_order: { "field": "has_placed_order", "yes_action": "continue", "no_action": "end_automation" } — checks if contact has ordered since enrolment
- opened_previous: { "field": "opened_previous", "yes_action": "continue", "no_action": "end_automation" } — checks if previous email was opened
- clicked_previous: { "field": "clicked_previous", "yes_action": "continue", "no_action": "end_automation" } — checks if previous email was clicked
- contact_type_is: { "field": "contact_type_is", "value": "retail", "yes_action": "continue", "no_action": "end_automation" } — checks contact type matches value
- pipeline_stage_is: { "field": "pipeline_stage_is", "value": "Qualified", "yes_action": "continue", "no_action": "change_pipeline_stage", "no_action_value": "Lost" } — checks pipeline stage matches value

Only contact_type_is and pipeline_stage_is use "value". The other three are boolean checks with no value field.
...
Valid fields: contact.types, contact.status, contact.source, contact.total_spend, contact.order_count, contact.email, contact.business_name
...
- Return ONLY valid JSON. No markdown fences, no explanation, no commentary.${productContext}${discountContext}${body.audience_hints ? `\n\nAudience context: ${body.audience_hints}` : ""}`;
  ...
  try {
    ...
    // Consume credits
    await consumeAiCredits(roaster.id as string, "generate_automation");
