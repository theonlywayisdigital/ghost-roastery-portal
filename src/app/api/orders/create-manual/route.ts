      ...
      if (itemBlendComps && itemBlendComps.length > 0 && weightGrams && weightGrams > 0) {
        ...
        for (const comp of itemBlendComps) {
          ...
          await deductFromPool(
            ...
            compKg,
            `Manual order ${order.id.slice(0, 8).toUpperCase()} — ${item.name} x ${item.quantity} (blend ${comp.percentage}%)`
          );
        }
      } else {
        ...
        if (roastedStockId && weightGrams && weightGrams > 0) {
          ...
          await deductFromPool(
            roastedStockId,
            deductKg,
            `Manual order ${order.id.slice(0, 8).toUpperCase()} — ${item.name} x ${item.quantity}`
          );
        }
      }
    }
